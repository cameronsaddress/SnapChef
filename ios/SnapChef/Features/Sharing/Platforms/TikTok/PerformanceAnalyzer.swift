...
public final class PerformanceAnalyzer: NSObject, @unchecked Sendable {
    ...
    private func logSessionSummary(_ session: SessionMetrics) {
        ...
        📊 Session Summary:
           ...
           Avg Memory: \(String(format: "%.1f", session.averageMemoryUsage / 1024 / 1024))MB
        """)
    }
    ...
    private func logRenderingMetricSummary(_ metric: RenderingMetric) {
        ...
        🎬 Rendering Complete:
           ...
           Max Memory: \(String(format: "%.1f", Double(maxMemory) / 1024 / 1024))MB
           File Size: \(metric.outputFileSize.map { String(format: "%.1f", Double($0) / 1024 / 1024) + "MB" } ?? "N/A")
    ...
    private func generateRecommendations(_ metrics: [RenderingMetric]) -> [String] {
        ...
        if avgMemory > Double(ExportSettings.maxMemoryUsage) * 0.8 {
            recommendations.append("Memory usage (\(String(format: "%.1f", avgMemory / 1024 / 1024))MB) is approaching limit. Consider memory optimization.")
        }
        
        // Check file sizes
        let avgFileSize = calculateAverageFileSize(metrics)
        if avgFileSize > Double(ExportSettings.targetFileSize) {
            recommendations.append("Average file size (\(String(format: "%.1f", avgFileSize / 1024 / 1024))MB) exceeds target. Consider compression optimization.")
        }
