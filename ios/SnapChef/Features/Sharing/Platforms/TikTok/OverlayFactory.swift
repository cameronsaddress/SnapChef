...
public extension OverlayFactory {
    /// SPEED OPTIMIZATION: Fast overlay application with cancellation support
    func applyOverlays(videoURL: URL,
        ...
        if overlays.count <= 3 {
            return try await applyOverlaysLightweight(videoURL: videoURL, overlays: overlays, cancellationToken: cancellationToken, progress: progress)
        }
        
        // Check memory status before heavy processing
        let memoryStatus = MemoryOptimizer.shared.getMemoryStatus()
        ...
        for (index, ov) in overlays.enumerated() {
            // Check for cancellation before processing each overlay
            try cancellationToken?.throwIfCancelled()
            
            // Monitor memory pressure during processing
            if index % 2 == 0 { // Check every other overlay
                let memoryStatus = MemoryOptimizer.shared.getMemoryStatus()
                if memoryStatus.pressureLevel == .critical {
                    throw CancellationError()
    ...
    private func applyOverlaysLightweight(videoURL: URL, overlays: [RenderPlan.Overlay], cancellationToken: CancellationToken? = nil, progress: @escaping @Sendable (Double) async -> Void) async throws -> URL {
