...
struct SnapChefApp: App {
    ...
    @StateObject var gamificationManager = GamificationManager()
        ...
        WindowGroup {
            ContentView()
                ...
                .sheet(isPresented: $showDeepLinkRecipe) {
                    DeepLinkRecipeView(socialShareManager: SocialShareManager.shared, cloudKitSync: CloudKitSyncService.shared)
                }
                .onReceive(NotificationCenter.default.publisher(for: UIApplication.willResignActiveNotification)) { _ in
                    // App is going to background
                    Task {
                        await CloudKitDataManager.shared.endAppSession()
                    }
                }
    ...
    private func setupApp() {
        ...
        Task {
            ...
            let sessionID = await CloudKitDataManager.shared.startAppSession()
            appState.currentSessionID = sessionID
            
            // Register device for multi-device sync
            try? await CloudKitDataManager.shared.registerDevice()
            
            // Perform initial sync
            await CloudKitDataManager.shared.performFullSync()
            
            // Track app launch
            await CloudKitDataManager.shared.trackScreenView("AppLaunch")
        }
    }
    ...
    private func handleIncomingURL(_ url: URL) {
        if SocialShareManager.shared.handleIncomingURL(url) {
            // URL was handled successfully
            showDeepLinkRecipe = true
            SocialShareManager.shared.resolvePendingDeepLink()
        }
    }
}
...
struct DeepLinkRecipeView: View {
    let socialShareManager: SocialShareManager
